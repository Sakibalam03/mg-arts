...
const Navigation = () => {
  ...
  const menuButtonRefs = useRef<(HTMLButtonElement | HTMLAnchorElement | null)[]>([]);

  const isTouchDevice = useIsTouchDevice();

  const handleMenuEnter = (hasSubmenu: boolean, index: number) => {
    ...
    if (e.key === 'ArrowDown' || (e.key === 'Tab' && !e.shiftKey)) {
      if (activeMenuIndex === index && submenuContainerRef.current) {
        ...
        submenuContainerRef.current
          ...
          ?.querySelector<HTMLElement>(`.${SUBMENU_LINK_CLASSNAME}`)
          ?.focus();
      }
    }
  };
  ...
  const handleMenuClick = useCallback(
    (hasSubmenu: boolean, index: number) => (e: React.MouseEvent) => {
      if (!hasSubmenu) return;
  ...
  const handleEscapeKey = useCallback(
    (e: KeyboardEvent) => {
      if (e.key === 'Escape' && activeMenuIndex !== null) {
        ...
        (menuButtonRefs.current[activeMenuIndex] as HTMLElement | null)?.focus();
      }
    },
    [activeMenuIndex]
  );

  const handleSubmenuNavigation = useCallback((containerIndex: number) => {
    let links: NodeListOf<HTMLElement> | undefined;

    if (submenuContainerRef.current) {
      links = submenuContainerRef.current
        .querySelector(`#submenu-${containerIndex}`)
        ?.querySelectorAll<HTMLElement>('.main-navigation-submenu-link');
    }
    ...
    return (e: React.KeyboardEvent) => {
      const menuTrigger = menuButtonRefs.current[containerIndex] as HTMLElement | null;
      const nextMenuTrigger = (menuButtonRefs.current[containerIndex + 1] as HTMLElement | null) ?? null;

      if (linkIndex === -1) {
        links!.forEach((link, idx) => {
          if (link === e.target) linkIndex = idx;
        });
      }
  ...
  const buttonRefObjects = menuButtonRefs.current
    .filter((r): r is HTMLButtonElement | HTMLAnchorElement => r !== null)
    .map((r) => ({ current: r as Element }));

  useClickOutside(
    [submenuContainerRef, ...buttonRefObjects],
    (e) => {
      if (e.target === menuButtonRefs.current[activeMenuIndex ?? -1]) return;
  ...
  useEffect(() => {
    if (submenuContainerRef.current) {
      const activePanel = submenuContainerRef.current.querySelector<HTMLElement>('[data-submenu-panel].active');
          ...
          return (
            ...
            >
              <Button
                ...
                onClick={handleMenuClick(hasSubmenu, index) as React.MouseEventHandler<HTMLButtonElement>}
              >
                {text}
                {hasSubmenu && (
                  <ChevronIcon
