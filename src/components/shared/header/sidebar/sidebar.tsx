...
const Sidebar = ({ isClient, isDocs, className }: SidebarProps) => (
  <div className={cn('flex items-center max-lg:hidden', isDocs ? 'gap-x-6' : 'gap-x-8', className)}>
    <div className={cn('flex items-center', isDocs ? 'gap-x-4' : 'gap-x-6')}>
      {SOCIALS.map((social) => {
        const { id, to, icon: Icon } = social;
        const label = 'label' in social ? social.label : undefined;
        const hasStars = 'hasStars' in social ? social.hasStars : false;
        return (
          <Link
            className={cn(
