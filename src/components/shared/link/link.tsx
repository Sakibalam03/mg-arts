...
import ArrowRightIcon from 'icons/arrow-right.inline';
import ExternalIcon from 'icons/external.inline';
import GlossaryIcon from 'icons/glossary.inline';
...
type IconKey = 'external' | 'glossary';
...
const Link = forwardRef<HTMLAnchorElement, LinkProps>(
  (
    ...
    const className = cn(
      ...
      withArrow && 'group inline-flex w-fit items-center gap-1 max-sm:wrap-anywhere',
    ...
    if (to?.startsWith('/')) {
      return (
        <NextLink
          ...
          prefetch={prefetch}
          onClick={handleClick}
          {...(props as Omit<React.ComponentPropsWithoutRef<typeof NextLink>, 'href' | 'className' | 'prefetch' | 'onClick'>)}
        >
          {content}
        </NextLink>
      );
