...
type Card = (typeof ROW_1)[0]

function TestimonialCard({ quote, name, city, avatar }: Card) {
  return (
...
function MarqueeRow({
  ...
  useEffect(() => {
    ...
    halfW.current = ref.current.scrollWidth / 2
  ...
  useAnimationFrame((_, delta) => {
    ...
    if (direction === "left") {
      ...
      const next = x.get() + (delta / 1000) * speed
      x.set(next >= 0 ? next - halfW.current : next)
    }
  })
  ...
  return (
    ...
    >
      <motion.div ref={ref} className="flex gap-3 w-max" style={{ x }}>
        {doubled.map((item, i) => (
  ...
  return (
    <section className="bg-[#050505] border-t border-[#111] py-20 max-sm:py-12 overflow-hidden">
      ...
      <div className="flex flex-col gap-3">
        <MarqueeRow items={ROW_1} direction="left" speed={46} />
        <MarqueeRow items={ROW_2} direction="right" speed={38} />
      </div>
    </section>
  )
}
